...
MOV R5, R1 ;save the total number of guesses to use later
...
PUSH {R0,R1}
MOV R0, #querycode
SUB R1, R5, R1  ;calculate the current guess number
BL drawPixel
POP {R0,R1}
...
getcode:
    ...
    ADD R3, R3, #1 ; increment the index
    CMP R3, #4 ; check if the index is 4
    BLT testloop ;  if it is less than 4 then check the next code
...
drawPixel:
    PUSH {R2,R3,R4,R5,R6,R7}
    MOV R2, R0 ;query code
    MOV R3, R1 ;current guess number
    MOV R7, #.PixelScreen


    calculatePixelLocationLoop:
        CMP R3, #0 
        BEQ finishCalculating
        ADD R7, R7, #256
        SUB R3, R3, #1
        B calculatePixelLocationLoop
    ...
    finish:
        ...
        ADD R7, R7, #4
        ADD R4, R4, #1
        CMP R4, #4 ; check if the index is 4
        BLT drawPixelLoop ;  if it is less than 4 then check the next code
